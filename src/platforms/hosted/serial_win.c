...
static char *read_key_from_path(const char *const subpath, const char *const key_name)
{
	...
	DWORD value_len = 0;
	const LSTATUS result = RegGetValue(key_path_handle, NULL, key_name, RRF_RT_ANY, NULL, NULL, &value_len);
	...
	assert(RegGetValue(key_path_handle, NULL, key_name, RRF_RT_ANY, NULL, value, &value_len) == ERROR_SUCCESS);
	RegCloseKey(key_path_handle);
	return value;
}
